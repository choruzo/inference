...
docker_compose() {
  if docker compose version >/dev/null 2>&1 && docker ps >/dev/null 2>&1; then
    env LLM_BASE_URL="http://host.docker.internal:${LLAMA_PORT}/v1" docker compose "$@"
    return
  fi

  if sudo -n true >/dev/null 2>&1; then
    if sudo -n env LLM_BASE_URL="http://host.docker.internal:${LLAMA_PORT}/v1" docker compose "$@"; then
      return
    fi
  fi

  if [[ -t 0 ]]; then
    sudo env LLM_BASE_URL="http://host.docker.internal:${LLAMA_PORT}/v1" docker compose "$@"
    return
  fi
